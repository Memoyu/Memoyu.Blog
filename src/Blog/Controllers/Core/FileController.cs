...
namespace Blog.Controllers.Core
{
    ...
    public class FileController : ApiControllerBase
    {
        ...
        {
            _fileSvc = componentContext.ResolveNamed<IFileSvc>(Appsettings.FileStorage.ServiceName);
        }
        ...
        public async Task<ServiceResult<FileDto>> UploadFile(IFormFile file, string type, int key = 0)
        {
            ...
            this.ValidFile(file);
            return ServiceResult<FileDto>.Successed(await _fileSvc.UploadAsync(file, type, key));
        }
        ...
        public async Task<ServiceResult<List<FileDto>>> UploadFiles(string type)
        {
            ...
            foreach (var file in files)
            {
                FileDto fileDto = await _fileSvc.UploadAsync(file, type, i++);
