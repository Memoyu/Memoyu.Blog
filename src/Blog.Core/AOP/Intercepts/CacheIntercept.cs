...
namespace Blog.Core.AOP.Intercepts
{
    public class CacheIntercept : IInterceptor
    {
            ...
            try
            {
                ...
                if (cacheAttrObj is CacheableAttribute cacheAttr)//如果为CacheableAttribute
                {
                    ...
                    if (cacheValue != null)//存在缓存数据
                    {
                        ...
                        {
                            dynamic d = JsonConvert.DeserializeObject(cacheValue, resultTypes.FirstOrDefault() ?? typeof(object));//将缓存反序列化为resultTypes.FirstOrDefault()类型
                    ...
                    {
                        var expire = cacheAttr.Expire <= 0 ? Appsettings.CacheExpire : cacheAttr.Expire;
                        if (invocation.ReturnValue is ServiceResult res)
                        {
                            if (res.Success)
                                RedisHelper.Set(cacheKey, JsonConvert.SerializeObject(invocation.ReturnValue), expire);
                        }
                        else
                        {
                            RedisHelper.Set(cacheKey, JsonConvert.SerializeObject(invocation.ReturnValue), expire);
        ...
        // 异步返回Task<T>
        private async Task<T> InterceptAsync<T>(string cacheKey, Task<T> task) where T : ServiceResult
        {
